...
서비스 개발이 1할이라면 서비스 유지 보수는 9할을 차지한다고 말해도 과언이 아닐 겁니다. 서비스 개발 과정에서 어떻게 API를 설계함에 따라 차후 유지 보수에 걸리는 시간은 천차만별입니다. 제가 `reactnative`로 간단한 앱을 만드는 과정에서 고민하게 된 지속 가능한 개발 방식에 대해서 이야기 하려고 합니다.
